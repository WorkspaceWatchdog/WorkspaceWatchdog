...
/**
 * Google Workspace Login Monitor v3.4.7
...
function fillBlankGeoInMain() {
  ...
  data.forEach((r, i) => {
    ...
    if (!g || g.source === 'failed' || !g.city) {
      try { g = _geoLookup_(ip); } catch(e) { g = null; }
    }
    if (!g || !g.city) return;
...
// Repo: https://github.com/WorkspaceWatchdog/WorkspaceWatchdog
// ============================================================
...
// ===== Year-End Data Reset ====================================================
//
// Clears all data rows (preserving headers) from Main, OUCache, Active Now,
// and Suspicious. Also clears the KeyIndex sheet and purges alert/digest
// dedup keys from Script Properties, and resets lastRunISO so the next sync
// uses the full lookback window. GeoCache, Archive, Diagnostics, Setup/Config
// values, and triggers are intentionally preserved.
//
// Requires the user to type RESET into a confirmation dialog to proceed.
// Logs the action to the Diagnostics sheet.

/**
 * Shows the Year-End Data Reset confirmation dialog.
 * Called from the Advanced submenu in the Workspace Watchdog menu.
 */
...
/**
 * Performs the year-end data reset. Called from the confirmation dialog
 * via google.script.run. Returns a status message string on success.
 *
 * Clears data rows (row 2 onward) on: Main, OUCache, Active Now, Suspicious.
 * Clears the KeyIndex sheet entirely (no header — it is key-only).
 * Resets lastRunISO and all ww_alert_, ww_alerted_, ww_chat_digest_,
 * and ww_weekly_report_ Script Properties.
 * Logs the action to Diagnostics.
 */
function executeYearEndReset() {
  ...
  // ── 1. Clear data rows on the four target sheets (preserve row 1 header) ──
  var targetSheets = [
  ...
  targetSheets.forEach(function(def) {
    ...
    if (!sh) return; // sheet missing — skip safely
  ...
  // ── 2. Clear the KeyIndex sheet entirely (it has no header row) ──
  var shKI = ss.getSheetByName(KEY_INDEX_SHEET);
  if (shKI) {
    if (shKI.getLastRow() > 0) {
      shKI.clearContents();
    }
  }
  // Reset the in-memory KeyIndex cache
  __KEY_INDEX = null;

  // ── 3. Reset all in-memory caches (Main and OUCache are now empty) ──
  _resetAllCaches_();

  // ── 4. Purge dedup Script Properties and reset the sync cursor ──
  var p = PropertiesService.getScriptProperties();
  var allKeys = p.getKeys();
  var purgedCount = 0;
  allKeys.forEach(function(k) {
    if (
      k.startsWith('ww_alert_')         ||
      k.startsWith('ww_alerted_')       ||
      k.startsWith('ww_chat_digest_')   ||
      k.startsWith('ww_weekly_report_')
    ) {
      p.deleteProperty(k);
      purgedCount++;
    }
  ...
  // Reset the sync cursor so the next scheduled sync uses the full lookback window
  p.deleteProperty('lastRunISO');

  // ── 5. Log the reset to Diagnostics ──
  var notes =
  ...
  try {
    _logDiagnostics('yearEndReset', now, new Date(), 0, 0, notes);
  } catch (e) {
    // Diagnostics logging failure should not prevent reset completion
  }

  // ── 6. Toast confirmation and return success message to the dialog ──
  ss.toast(
    'Year-end reset complete. ' + totalRowsCleared + ' rows cleared across ' +
    targetSheets.length + ' sheets.',
